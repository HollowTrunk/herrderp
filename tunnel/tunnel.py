...
class Road(pygame.sprite.Group):
	...
	def new_size(self, input):
		result = int((320/math.exp(input)+20))
		return result

	def defill(self):
		self.size[0] = self.new_size(self.size[0])

		self.chunk_list.insert(0, Chunk(position, self.size))
		self.chunk_list.pop()

		for i in len(self.chunk_list):
			position = (160, (0 + 20*i))
